...
export default function PeoplePageClient({ published, regions }: { published: Person[]; regions: Region[] }) {
  const { locale, t } = useLocale();

  // Group by era (mutually exclusive ranges)
  const ancient = published.filter((p) => (p.birthYear ?? 0) < 0);
  const earlyMedieval = published.filter((p) => {
    const by = p.birthYear ?? 0;
    return by >= 0 && by < 960;
  });
  const songEra = published.filter((p) => {
    const by = p.birthYear ?? 0;
    return by >= 960 && by < 1127;
  });
  const postSong = published.filter((p) => {
    const by = p.birthYear ?? 0;
    return by >= 1127 && by < 1500;
  });
