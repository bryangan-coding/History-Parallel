#!/usr/bin/env python3
"""Generate person entries for NEW dynasties (not covered by the main batch script):
民国, 五代, 十国, 辽/金/西夏, 两晋, 十六国

Usage:
  python3 scripts/generate-people-cbdb-new-dynasties.py > /tmp/cbdb_new_dynasties.ts
"""
...
DYNASTY_CONTEXT = {
    ...
    '南平': '南平（荆南，924—963年）是十国之一，高季兴所建，据有荆州地区，以善事大国著称。',
    ...
    '前燕': '前燕（337—370年）是十六国之一，鲜卑慕容氏建立于辽东，后迁都邺城，被前秦所灭。',
    '後秦': '后秦（384—417年）是十六国之一，羌族姚苌所建，据有关中地区，推崇佛教，被东晋刘裕所灭。',
    '南燕': '南燕（398—410年）是十六国之一，鲜卑慕容德建立于山东，被东晋所灭。',
    '前涼': '前凉（317—376年）是十六国之一，汉人张轨在河西走廊建立的政权，是十六国中存续最长者之一。',
    '西涼': '西凉（400—421年）是十六国之一，汉人李暠在敦煌建立的政权，后被北凉所灭。',
    '前秦': '前秦（350—394年）是十六国之一，氐族苻洪建立，苻坚时期一度统一北方，淝水之战后崩溃。',
    '後燕': '后燕（384—407年）是十六国之一，鲜卑慕容垂建立于河北，后被北燕取代。',
    '後趙': '后赵（319—351年）是十六国之一，羯族石勒所建，一度统治北方大部分地区。',
...
DYNASTY_CONTEXT_EN = {
    ...
    '五代': 'The Five Dynasties (907–960) were five successive short-lived regimes in the Central Plains after the fall of Tang, a transitional period from division to reunification.',
    ...
    '南平': 'Nanping (Jingnan, 924–963) was one of the Ten Kingdoms, founded by Gao Jixing in the Jingzhou region, known for skillful diplomacy.',
    ...
    '後秦': 'Later Qin (384–417) was one of the Sixteen Kingdoms, founded by the Qiang leader Yao Chang in the Guanzhong region, known for promoting Buddhism.',
    '南燕': 'Southern Yan (398–410) was one of the Sixteen Kingdoms, founded by the Xianbei Murong De in Shandong.',
    '前涼': 'Former Liang (317–376) was one of the Sixteen Kingdoms, a Han Chinese regime founded by Zhang Gui in the Hexi Corridor.',
    '西涼': 'Western Liang (400–421) was one of the Sixteen Kingdoms, founded by the Han Chinese Li Gao at Dunhuang.',
    '前秦': 'Former Qin (350–394) was one of the Sixteen Kingdoms, founded by the Di leader Fu Hong; under Fu Jian it briefly unified the north before the Battle of Fei River.',
    '後燕': 'Later Yan (384–407) was one of the Sixteen Kingdoms, founded by the Xianbei Murong Chui in Hebei.',
    '後趙': 'Later Zhao (319–351) was one of the Sixteen Kingdoms, founded by the Jie leader Shi Le, once dominating most of northern China.',
    '後涼': 'Later Liang (386–403) was one of the Sixteen Kingdoms, a Di regime founded by Lü Guang in Liangzhou.',
    '前趙': 'Former Zhao (304–329) was one of the Sixteen Kingdoms, founded by the Xiongnu Liu Yuan, one of the earliest regimes of the period.',
    '西秦': 'Western Qin (385–431) was one of the Sixteen Kingdoms, founded by the Xianbei Qifu Guoren in Longxi.',
    '北燕': 'Northern Yan (407–436) was one of the Sixteen Kingdoms, a Han Chinese regime founded by Feng Ba in western Liaoning.',
    '北涼': 'Northern Liang (401–439) was one of the Sixteen Kingdoms, founded by the Xiongnu Juqu Mengxun in the Hexi Corridor.',
    '西燕': 'Western Yan (384–394) was one of the Sixteen Kingdoms, founded by the Xianbei Murong Chong in the Guanzhong region.',
    '代': 'The Dai state (310–376) was one of the Sixteen Kingdoms, established by the Xianbei Tuoba clan south of the Gobi, the predecessor of Northern Wei.',
}

# Dynasty → regionId mapping (DB dynasty_chn → our region ID)
DYN_REGION = {
    ...
    '前蜀': 'ten-kingdoms',
    '遼': 'liao-dynasty',
    '金': 'jin-dynasty-period',
    '偽齊': 'jin-dynasty-period',
    '西遼': 'liao-dynasty',
    '西夏': 'western-xia',
    '西晉': 'western-jin',
    '東晉': 'eastern-jin',
...
# Display name mapping (DB dynasty_chn → display name for tags)
DYN_DISPLAY = {
    '中華民國': '民国', '中華人民共和國': '中国',
    '五代': '五代', '後梁': '后梁', '後唐': '后唐', '後晉': '后晋',
    '後漢': '后汉', '後周': '后周',
    ...
    '西晉': '西晋', '東晉': '东晋', '晉': '晋',
    '前燕': '前燕', '後秦': '后秦', '南燕': '南燕', '前涼': '前凉',
    '西涼': '西凉', '前秦': '前秦', '後燕': '后燕', '後趙': '后赵',
    '後涼': '后凉', '前趙': '前赵', '西秦': '西秦', '北燕': '北燕',
    '北涼': '北凉', '西燕': '西燕', '代': '代国',
}

# DB path and mockData path
DB_PATH = '/tmp/cbdb_20260530.sqlite3'
MOCKDATA_PATH = 'src/data/mockData.ts'
...
cursor.execute(f"""
    ...
    AND (
        (b.c_birthyear > 0 AND b.c_deathyear > 0)
        OR (b.c_birthyear > 0 AND b.c_deathyear <= 0)
        OR (b.c_birthyear <= 0 AND b.c_deathyear > 0)
        OR (b.c_index_year > 0 AND b.c_birthyear <= 0 AND b.c_deathyear <= 0)
    )
...
for row in candidates:
    ...
    if '宋' in display_dyn and num_postings >= 5:
        tags.insert(1, '文人')
        tags_en.insert(1, 'Scholar-Official')
        if '文人' not in occs:
            occs.insert(0, '文人')
...
# Output
print(f"// Auto-generated historical figures from new dynasties (民国/五代/十国/辽金西夏/两晋/十六国)")
print(f"// Data sourced from historical records, transformed for CC BY-SA 4.0 compliance")
print(f"// Total: {len(entries)} entries")
print(f"export const _newDynastiesPeople: Person[] = [")
print('\n'.join(entries))
print("];")
