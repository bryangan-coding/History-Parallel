 ...
 * 
 * 策略改进：
 * 1. 同一人物同年（±2年）事件，只要共享核心关键词，就进入候选重复组
 * 2. 按质量评分排序，保留最优的一条
 * 3. 特别处理 evt-lifespan-* 模板 — 这些几乎都是垃圾
 */
...
async function main() {
  ...
  try {
      ...
      for (const [, yearEvents] of yearGroups) {
        ...
        for (let i = 0; i < yearEvents.length; i++) {
          ...
          for (let j = i + 1; j < yearEvents.length; j++) {
            ...
            // If keyword overlap > 0, they are about the same topic
            if (overlap > 0 || bothLifespan || bothStruct) {
    ...
    fs.writeFileSync('scripts/dedup_v3_report.json', JSON.stringify({
    ...
    console.log(`\n完整报告已保存到 scripts/dedup_v3_report.json`);
    ...
    sqlLines.push('');
    sqlLines.push('-- Verify after delete:');
    sqlLines.push("-- SELECT COUNT(*) FROM events WHERE data_status = 'published';");
    
    fs.writeFileSync('scripts/dedup_v3.sql', sqlLines.join('\n'));
    console.log(`SQL 已保存到 scripts/dedup_v3.sql`);
